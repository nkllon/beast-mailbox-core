...
class PRDiagnostic:
    ...
    def get_workflow_runs(self) -> List[Dict[str, Any]]:
        ...
        params = {"head_sha": self.get_pr_info()["head"]["sha"], "per_page": 100}
    ...
    def analyze_dependency_changes(self, files: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        dependency_changes = {"pyproject_toml": [], "workflow_files": [], "other": []}
            ...
            if filename == "pyproject.toml":
                ...
                for line in lines:
                    if (
                        line.startswith("+")
                        and "=" in line
                        and ("version" in line.lower() or "==" in line or ">=" in line)
                    ):
                        dependency_changes["pyproject_toml"].append(
                            {"line": line.strip(), "change": "added"}
                        )
                    elif (
                        line.startswith("-")
                        and "=" in line
                        ...
                        dependency_changes["pyproject_toml"].append(
                            {"line": line.strip(), "change": "removed"}
                        )
                ...
                ext in filename for ext in [".lock", "requirements", "poetry.lock"]
            ):
                ...
                if self.token:
                    try:
                        ...
                        if jobs_response.status_code == 200:
                            ...
                            failed_jobs = [
                                j for j in jobs if j["conclusion"] == "failure"
                            ]
                            if failed_jobs:
                                failure["failed_jobs"] = [
                                    {
    ...
    def print_report(self, report: Dict[str, Any]):
        ...
        if report["files_changed"]["files"]:
            ...
            for file in report["files_changed"]["files"][:10]:
                print(
                    f"    {file['status']}: {file['name']} (+{file['additions']}, -{file['deletions']})"
                )
