...
class TestFetchLatestMessages:
    ...
    @pytest.mark.asyncio
    async def test_fetch_ack_handles_busygroup(
        self, mock_service, sample_redis_messages
    ):
    ...
    @pytest.mark.asyncio
    async def test_fetch_ack_handles_non_busygroup_error(
        self, mock_service, sample_redis_messages
    ):
        """Test ack logs warning for non-BUSYGROUP errors."""
        service, client = mock_service
        client.xrevrange = AsyncMock(return_value=sample_redis_messages)
        client.xgroup_create = AsyncMock(side_effect=Exception("Permission denied"))
        client.xack = AsyncMock(return_value=2)
    ...
    @pytest.mark.asyncio
    async def test_fetch_ack_failure_raises_systemexit(
        self, mock_service, sample_redis_messages
    ):
    ...
    @pytest.mark.asyncio
    async def test_fetch_trim_failure_raises_systemexit(
        self, mock_service, sample_redis_messages
    ):
