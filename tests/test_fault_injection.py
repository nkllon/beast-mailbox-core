...
@pytest.mark.asyncio
@pytest.mark.skipif(
    not pytest.importorskip("redis"), reason="Redis library not available"
)
async def test_start_with_existing_consumer_group_busygroup(
    fault_injection_config, fault_agent_id, redis_available, redis_docker
):
...
async def test_recovery_with_no_consumer_group_nogroup(
    fault_injection_config, fault_agent_id, redis_available, redis_docker
):
...
async def test_acknowledge_messages_with_existing_group_busygroup(
    fault_injection_config, fault_agent_id, redis_available, redis_docker
):
...
async def test_recovery_callback_handles_exception(
    fault_injection_config, fault_agent_id, redis_available, redis_docker
):
    ...
    service = RedisMailboxService(
        fault_agent_id, fault_injection_config, recovery_callback=callback
    )
    service.register_handler(handler)
...
async def test_handler_exception_during_recovery(
    fault_injection_config, fault_agent_id, redis_available, redis_docker
):
