...
class TestRunServiceAsyncLatestMode:
    ...
    async def test_run_service_async_latest_mode(self):
        ...
        with patch(
            "beast_mailbox_core.cli._fetch_latest_messages", new_callable=AsyncMock
        ) as mock_fetch:
...
class TestAdditionalCoverage:
    ...
    NOTE: We removed heavily mocked tests that didn't actually test real behavior.
    Error handling for CLI is tested via integration tests with real Redis.
    """
