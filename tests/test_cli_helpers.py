...
class TestAcknowledgeMessages:
    ...
    async def test_acknowledge_creates_group_and_acks(self):
        ...
        await _acknowledge_messages(
            mock_client, "test:stream", "test:group", [b"msg1", b"msg2", b"msg3"]
        )

        mock_client.xgroup_create.assert_called_once()
        mock_client.xack.assert_called_once_with(
            "test:stream", "test:group", b"msg1", b"msg2", b"msg3"
        )
    ...
    async def test_acknowledge_with_non_busygroup_error(self):
        ...
        await _acknowledge_messages(mock_client, "test:stream", "test:group", [b"msg1"])
...
class TestTrimMessages:
    ...
    async def test_trim_deletes_messages(self):
        ...
        await _trim_messages(mock_client, "test:stream", [b"msg1", b"msg2"])
