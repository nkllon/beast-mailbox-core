...
Redis-backed mailbox utilities extracted from Beast Mode, now with a same-host filesystem transport for lightweight deployments.

## Highlights
- Redis and filesystem backends with a consistent async API
- Atomic JSON persistence for same-host agents
- CLI tooling for message inspection, sending, and service orchestration
- SonarCloud-quality gated with maintainability and coverage reporting
...
- Optional file system mailbox for co-located Beast agents
...
### File System Mailbox (Same-Host Agents)

When all Beast agents share a local file system, use the file system mailbox:

```python
from beast_mailbox_core import FileSystemMailboxService, FileSystemMailboxConfig

config = FileSystemMailboxConfig(base_path="/var/beast/mailbox")
service = FileSystemMailboxService("my-agent", config=config)
...
Environment variables provide easy overrides:

- `BEAST_MAILBOX_FS_ROOT` (default: `/tmp/beast_mailbox`)
- `BEAST_MAILBOX_FS_POLL_INTERVAL` (default: `0.5`)
- `BEAST_MAILBOX_FS_MKDIR_MODE` (default: `755`)

The service writes messages as JSON files under
`{base_path}/{agent_id}/inbox` and safely handles concurrent readers.
