...
- The system SHALL target macOS 15.0 or later (for Apple Intelligence support).
...
## 2.1. Apple Intelligence Integration
- The system SHALL provide a chat interface that integrates with on-device Apple Intelligence.
- The system SHALL check Apple Intelligence availability and provide appropriate fallback messages when unavailable.
- The system SHALL support specialized query types including:
  - Code review and analysis
  - Error diagnosis and debugging assistance  
  - Software architecture guidance
  - Documentation generation
  - General development assistance
- The system SHALL automatically detect query intent and apply appropriate specialized instructions.
- The system SHALL use the FoundationModels framework for direct access to Apple Intelligence capabilities.
- The system SHALL gracefully handle all Apple Intelligence unavailability scenarios (device not eligible, feature disabled, model not ready).
...
## 13. Traceability (Requirements → Artifacts)
- Menu bar UI states → `MenuBarView` (icon, color), `SyncStatusMonitor` (state derivation).
- Sync orchestration (async, timeout) → `ObservatoryService.syncNow(timeout:)`, `SyncStatusMonitor.triggerSync()`.
- Last sync reporting → `SyncStatusMonitor.lastSyncDate`, file-based inference in `ObservatoryService.getLastSyncTime()`.
- Notifications → `AppDelegate` (authorization), `SyncStatusMonitor.sendNotification(...)`.
- Error handling (timeout, exit codes) → `ObservatoryError`, `SyncStatusMonitor.triggerSync()` mapping.
- Dependency injection → `ObservatoryServicing` protocol and initializer of `SyncStatusMonitor`.
- Logging → `Logger` usage in `AppDelegate`, `ObservatoryService`, and `SyncStatusMonitor`.

## 14. Variants & Deployment Options
- Process-first (default):
  - Pros: No network dependency, simple deployment.
  - Cons: Requires local binary at a known path; sandbox may restrict access.
- HTTP-first (optional):
  - Pros: Easier to mock in CI and debug; decouples app from host binary.
  - Cons: Requires service provisioning; network/port management; versioning of API.
- Hybrid:
  - Choose via environment variables or settings; fall back to process mode if HTTP is unavailable.

## 15. Cross-cutting Concerns
### 15.1 Security & Privacy
- No secrets are stored in the repository; agents SHALL use environment variables or secret managers for any credentials.
- Notifications contain minimal information; avoid sensitive content in notifications.
- If using HTTP mode, prefer localhost or secure channels; do not expose endpoints publicly without authentication.
- For App Store builds, review sandbox entitlements; direct access to `~/Library/Logs` may be disallowed.

### 15.2 Manageability & Operations
- Structured logs via `os.Logger` for key lifecycle events.
- Timeouts and error codes are surfaced clearly to users and logs.
- Periodic refresh tasks are cancellable to avoid leaks.
- Consider adding a simple in-app log viewer for sandboxed distributions.

### 15.3 Configuration
- Defaults:
  - Timeout: 60 seconds.
  - Local binary path: `/usr/local/bin/beast-observatory-sync`.
  - Log path: `~/Library/Logs/beast-observatory/sync.log`.
- Overrides may be provided via environment variables or future settings UI.
- Optional HTTP mode can be selected via env vars (see AGENTS.md example mechanism).

### 15.4 Observability
- Log sync start/finish, exit codes, timeouts.
- Log notification permission results and errors.
- Log attempts to open logs and any failures.

### 15.5 Accessibility & UX
- Status icon has accessibility labels.
- “Sync Now” is disabled while syncing.
- Errors are displayed inline and via notifications.

### 15.6 Licensing & Distribution
- License: MIT (free, OSS). Include standard disclaimer (AS IS, no warranty; YMMV).
- App Store: classify as an advanced developer tool.
  - Verify entitlements and sandboxing.
  - Verify notifications usage description.
  - Ensure any non-App-Store-allowed behaviors (e.g., executing external binaries) are gated or documented for non-Store builds.

---

Appendix A: Glossary
- Sync Process: The external mechanism that performs data synchronization.
- Last Sync Time: The most recent successful sync completion timestamp.
- Process Mode: Local binary execution.
- HTTP Mode: Service exposed over HTTP.
