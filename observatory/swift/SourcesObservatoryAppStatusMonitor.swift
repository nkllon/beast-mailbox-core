    ...
    func updateStatus() {
        Task {
            do {
                ...
                logger.debug("Status updated: lastSync=\(status.lastSyncTime?.description ?? "nil"), error=\(status.lastError ?? "none")")
            } catch {
                // Connection errors are expected when service isn't running
                if !error.localizedDescription.contains("Could not connect") &&
                   !error.localizedDescription.contains("Connection refused") {
                    ...
                    // Service not running - this is fine, just use log file fallback
                    logger.debug("Observatory service not running (expected), using log file")
                }
            }
        }
    }
