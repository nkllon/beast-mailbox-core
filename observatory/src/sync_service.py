...
try:
    ...
    from beast_mailbox_core import MailboxConfig, RedisMailboxService
...
class SonarCloudClient:
    ...
    async def get_historical_metrics(
        self, metrics: List[str], days: int = 30, page_size: int = 100
    ) -> Dict:
    ...
    async def get_current_metrics(self, metric_keys: List[str]) -> Dict:
        ...
        params = {"component": self.project_key, "metricKeys": ",".join(metric_keys)}
        ...
        try:
            ...
            async with self.session.put(
                ...
                if response.status == 200:
                    ...
                    logger.error(
                        f"Failed to push metrics (HTTP {response.status}): {error_text}"
                    )
                    return False
        except Exception as e:
...
async def convert_sonarcloud_to_prometheus(
    ...
    version_escaped = (
        version.replace("\\", "\\\\").replace('"', '\\"') if version else None
    )
    ...
    if (
        current_data
        and "component" in current_data
        and "measures" in current_data["component"]
    ):
        for measure in current_data["component"]["measures"]:
            ...
            try:
                ...
                logger.warning(
                    f"Non-numeric value for {prom_metric}: {value_obj}, skipping"
                )
                continue
        ...
        for measure in historical_data["measures"]:
            ...
            if history:
                ...
                try:
                    ...
                    logger.warning(
                        f"Non-numeric value for {prom_metric}: {value}, skipping"
                    )
                    continue
        ...
        try:
            ...
            if use_mailbox and HAS_MAILBOX and redis_config:
                ...
                mailbox_service = RedisMailboxService(
                    agent_id="observatory-sync", config=redis_config
                )
                publisher = MailboxPublisher(mailbox_service)
                await publisher.publish_metrics(
                    prom_metrics,
                    {"branch": branch, "version": version, "source": "sonarcloud-sync"},
                )
                ...
                async with MetricsPusher(pushgateway_url, pushgateway_auth) as pusher:
                    instance = (
                        f"{version}-sync-{int(time.time())}"
                        if version
                        else f"sync-{int(time.time())}"
                    )
