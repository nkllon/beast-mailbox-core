...
async def ask_apple_intelligence(
    ...
    if not pool_entry["started"]:
        ...
        async def handle_response(msg: MailboxMessage) -> None:
            if msg.message_type in (
                "APPLE_INTELLIGENCE_RESPONSE",
                "APPLE_INTELLIGENCE_ERROR",
            ):
    ...
    try:
        response_msg = await asyncio.wait_for(response_queue.get(), timeout=timeout)
...
async def review_code(
    code: str, query: str = "Review this code for issues and improvements"
) -> str:
    """
...
async def diagnose_error(
    error_log: str, query: str = "Why did this error occur and how do I fix it?"
) -> str:
    """
    ...
    """
    return await ask_apple_intelligence(
        query, context=error_log, query_type="error_diagnosis"
    )
...
async def get_architecture_advice(query: str, context: Optional[str] = None) -> str:
    ...
    """
    return await ask_apple_intelligence(
        query, context=context, query_type="architecture"
    )
