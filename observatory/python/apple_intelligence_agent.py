...
INTELLIGENCE_SERVER_URL = os.getenv(
    "APPLE_INTELLIGENCE_SERVER_URL", "http://localhost:8081"
)
...
class AppleIntelligenceAgent:
    ...
    def __init__(self, config: MailboxConfig):
        self.mailbox = RedisMailboxService(agent_id="apple-intelligence", config=config)
    ...
    async def handle_message(self, message: MailboxMessage) -> None:
        ...
        try:
            ...
            await self.mailbox.send_message(
                ...
                payload={"error": str(e), "original_query": query},
            )

    async def query_apple_intelligence(
        self, query: str, context: Optional[str] = None, query_type: str = "general"
    ) -> str:
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            response = await client.post(
                ...
                json={"query": query, "context": context, "query_type": query_type},
            )
