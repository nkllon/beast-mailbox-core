...
class RedisMailboxService:
    ...
    def __init__(
        ...
        recovery_callback: Optional[
            Callable[[RecoveryMetrics], Awaitable[None]]
        ] = None,
    ):
    ...
    async def _recover_pending_messages(self) -> RecoveryMetrics:
        ...
        try:
            ...
            self.logger.debug(
                "Checking pending messages: stream=%s, group=%s",
                self.inbox_stream,
            ...
            self.logger.debug(
                "xpending_range returned: %s (type: %s, len: %s)",
                pending_info,
                type(pending_info),
                len(pending_info) if pending_info else 0,
            )
            ...
            if "NOGROUP" in str(exc):
                self.logger.debug(
                    "Consumer group does not exist yet - skipping recovery"
                )
                metrics.end_time = asyncio.get_event_loop().time()
        ...
        while True:
            try:
                self.logger.debug(
                    "Calling xautoclaim with start_id=%s, min_idle_time=%dms",
                    start_id,
                    self.config.recovery_min_idle_time * 1000,
                )
                ...
                claimed_data = await self._client.xautoclaim(
                    ...
                    min_idle_time=self.config.recovery_min_idle_time
                    * 1000,  # Convert to ms
                    start_id=start_id,
                ...
                self.logger.debug(
                    "xautoclaim returned: %s (type: %s, len: %s)",
                    claimed_data,
                    type(claimed_data),
                    len(claimed_data) if claimed_data else None,
                )

                if not claimed_data or len(claimed_data) < 3:
                    self.logger.debug(
                        "No more claims available (claimed_data=%s), breaking",
                        claimed_data,
                    )
                    break
                ...
                self.logger.debug(
                    "Next start ID: %s (type: %s), Messages: %d",
                    next_start_id,
                    type(next_start_id),
                ...
                if not messages:
                    ...
                    self.logger.debug(
                        "No messages in this batch, next_start_id_str=%s",
                        next_start_id_str,
                    )
    ...
    async def start(self) -> bool:
        ...
        try:
            self.logger.debug(
                "Creating consumer group: stream=%s, group=%s",
                self.inbox_stream,
                self._consumer_group,
            )
    ...
    async def _dispatch(self, message: MailboxMessage) -> None:
        ...
        if not self._handlers:
            self.logger.info(
                "Mailbox message received with no handlers registered: %s", message
            )
            return
